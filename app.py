    ...
    font-weight: 600;
                ...
                if _has_alt_result and _da_has_conflicts and _da_has_load:
                    ...
                    with _tab_as:
                        ...
                        # Compute only when user explicitly presses "Calcular alternativas";
                        # render always from the stored photo so Apply doesn't change the list.
                        ...
                        if _da_btn_clicked:
                            ...
                            if (
                                ...
                                # Save photo and reset editor so checkboxes start fresh
                        ...
                        # Render from frozen photo; never re-derived from live source
                        if _as_foto is not None:
                            _dat_rows     = _as_foto["rows"]
                            _as_src_label = _as_foto["source_label"]
                            with st.container(border=True):
                                st.markdown("#### Alternativas de ajuste temporal")
                                if _as_src_label == "sim":
