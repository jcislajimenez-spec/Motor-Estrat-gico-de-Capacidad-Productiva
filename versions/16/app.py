...
def capacity_hours_for_output(merged: pd.DataFrame, output_units: float) -> float:
    """Horas totales requeridas para producir *output_units*.

    Nota: en este proyecto `cycle_time` es el **work content total por unidad** de cada
    proceso (en horas). Por tanto, las horas para *output_units* se calculan como:

        horas_totales = Σ (output_units * cycle_time_proceso)

    No se divide por estaciones u operarios aquí, porque ese reparto ya se usa para
    calcular la *capacidad* (uds/sem) a partir de horas disponibles.
    """
    if merged is None or merged.empty:
        return 0.0
    m = merged.copy()
    wc = pd.to_numeric(m.get("cycle_time"), errors="coerce").fillna(0.0)
    return float((output_units * wc).sum())
...
with tabs[2]:
    ...
    for line in lines:
        ...
        # Horas equivalentes requeridas (work content) para producir la DEMANDA semanal.
        # (Si más adelante quieres también horas para capacidad máxima, añadimos otra columna.)
        cap_hours_week = capacity_hours_for_output(merged, demand_week)
