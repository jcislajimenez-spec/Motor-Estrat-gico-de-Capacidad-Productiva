...
def capacity_hours_for_output(merged: pd.DataFrame, output_units: float) -> float:
    ...
    En TU modelo (por lo que estás validando en el pantallazo):
    - `cycle_time` ya está en **HH/ud** (lo pone también en la ayuda del editor).
    - Por tanto NO hay que multiplicar por operarios, porque eso duplicaría HH.

    Regla correcta:
        HH_total = sum_procesos( output_units * cycle_time )

    Nota:
    - Las estaciones/operarios sirven para calcular la **capacidad en uds/sem** (paralelización),
      pero NO para inflar el contenido de trabajo por unidad.
    """
