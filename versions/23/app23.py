...
with tabs[3]:
    ...
    if not line_stats_rows:
        ...
        H_max_plant = float(plant_max_h_sem) if 'plant_max_h_sem' in locals() else 0.0

        if H_max_plant <= 0:
            st.info("No hay techo estructural disponible (H_max_plant = 0). Revisa que existan líneas con capacidad > 0.")
        else:
            # 2) MaxH_m por modelo = suma de cap_h_sem(line, model) en todas las líneas compatibles (capacidad > 0)
            # Reutilizamos el diccionario capH_line_model construido en Nivel 2 (clave: (line_id, model) -> cap_h_sem).
            models_valid = sorted({m for (_, m) in capH_line_model.keys()})
            if not models_valid:
                st.info("No hay modelos válidos (capacidad estructural en horas = 0) para mostrar el simulador.")
            else:
                maxH_by_model = {}
                share_max_by_model = {}
                for m in models_valid:
                    total_h = 0.0
                    for (l_id, mm), h in capH_line_model.items():
                        if mm != m:
                            continue
                        try:
                            h_val = float(h)
                    ...
                    share_max_by_model[m] = (total_h / H_max_plant) if total_h > 0 else 0.0

                # Filtramos modelos con Share_m_max > 0 para que el slider tenga sentido
                models_valid = [m for m in models_valid if share_max_by_model.get(m, 0.0) > 0]
                if not models_valid:
                    st.info("Todos los modelos tienen capacidad estructural 0 (o no hay compatibilidades con capacidad > 0).")
                else:
                    import plotly.graph_objects as go

                    st.markdown("**Techo estructural planta (H_max_plant):** {:.2f} h/sem".format(H_max_plant))

                    # Controles + donuts por modelo
                    # (sliders independientes; no bloqueamos si la suma total supera 100%)
                    selected_share = {}
                    for m in models_valid:
                        share_max = float(share_max_by_model[m])  # 0..1
                        max_pct = share_max * 100.0
                        ...
                        selected = st.slider(
                            f"{m} — ocupación estructural simulada (% de planta)",
                            min_value=0.0,
                        ...
                        selected_share[m] = float(selected)

                        # Donut individual: % seleccionado vs % máximo posible del modelo
                        # (El donut "vive" dentro del máximo del modelo: seleccionado + resto hasta su máximo)
                        rest = max(0.0, max_pct - selected_share[m])
                        fig = go.Figure(
                            data=[
                                go.Pie(
                                    values=[selected_share[m], rest],
                                    labels=["Seleccionado", "Disponible hasta máx."],
                                    hole=0.68,
                                    sort=False,
                                    direction="clockwise",
                                    textinfo="none",
                        ...
                        fig.update_layout(
                            ...
                            annotations=[
                                dict(
                                    text=f"{selected_share[m]:.1f}%<br><span style='font-size:12px;color:#6b6b6b'>máx {max_pct:.1f}%</span>",
                                    x=0.5,
                                    y=0.5,
                                    font=dict(size=16),
                                    showarrow=False,
                    ...
                    # Indicador global agregado (termómetro / gauge)
                    total_pct = sum(selected_share.values())
                    exceso = max(0.0, total_pct - 100.0)

                    gauge_max = max(110.0, total_pct + 5.0)  # deja margen si se pasa
                    bar_color = "#b11226" if total_pct > 100.0 else "#2e7d32"  # rojo si exceso, verde si ok
                    ...
                    fig_total = go.Figure(
                        go.Indicator(
                            ...
                            value=total_pct,
                            number={"suffix": "%"},
                            title={"text": "Ocupación estructural agregada (suma de sliders)"},
                            gauge={
                                "axis": {"range": [0, gauge_max]},
                                "bar": {"color": bar_color},
                                "steps": [
                                    {"range": [0, 100], "color": "#e6e6e6"},
                                ],
                                "threshold": {
                                    "line": {"color": "#b11226", "width": 4},
                                    "thickness": 0.75,
                                    "value": 100,
                                },
                            },
                    ...
                    fig_total.update_layout(height=240, margin=dict(l=10, r=10, t=30, b=10))
                    st.plotly_chart(fig_total, use_container_width=True)

                    if exceso > 0:
                        st.error(f"Exceso estructural: {exceso:.1f}% (la suma supera el 100% del techo de planta).")
